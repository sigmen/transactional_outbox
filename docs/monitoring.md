...
* `worker.run` — published when a worker thread is spawned for a queue. Payload key is only `queue` (worker's queue).
* `worker.events.processed` — published after a worker successfully produces and deletes a batch of events. Payload keys are `queue` (worker's queue), `count` (processed events count). If event processing raises an error, `worker.events.processed` is not published for that cycle — the exception goes to the configured [failover](failover.md) instead.
* `worker.stopped` — published when a worker thread stops. Payload key is only `queue` (worker's queue).
* `worker.exceptions_total` — published whenever `EventProcessor#call` rescues a `StandardError` while fetching/producing/deleting events, right before it's forwarded to the configured [failover](failover.md). Payload keys are `queue` (worker's queue), `exception` (the exception's class).
...
TransactionalOutbox::Relay.monitor.subscribe("worker.events.processed") do |event|
  ...
  tags = { queue: event.payload[:queue] }

  metrics.worker.events_processed.set(tags, by: event.payload[:count])
end
```

NOTE: The monitor is implemented using [dry-monitor](https://hanakai.org/learn/dry/dry-monitor) instrumentation.
