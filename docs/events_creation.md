...
`TransactionalOutbox::Event` is a class that describes an event: which queue it goes to, what its payload looks like, and how the incoming data should be validated before it's persisted. You've to declare one event per each event type.
...
* **required** `queue` - a queue name where you need to put an event at end of processing.
* `queue_extra_parameters` - any extra queue options.
  ...
  module Events
    module User
      class CreatedEvent < TransactionalOutbox::Event
        ...
        event_type "created"
        queue "users"
        queue_extra_parameters { exchange_type: "fanout" }
...
Both simply insert into the outbox table — they don't open a transaction by themselves. To get the actual transactional-outbox guarantee (the event row committed atomically together with the business data change it describes), wrap the write with `TransactionalOutbox.outboxable`:
...
`TransactionalOutbox.outboxable` opens a database transaction through the configured [database adapter](database.md). As long as the rest of your business writes inside the block go through the same underlying connection (the `active_record`/`sequel` model configured in `db.connection_data`), they commit or roll back together with the outbox insert.
